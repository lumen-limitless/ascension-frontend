...
export default function Delegate() {
  ...
  const balance = useAscensionTokenBalanceOf({
  ...
  const votes = useAscensionTokenGetVotes({
    args: [address as `0x${string}`],
    enabled: !!address,
    watch: true,
    chainId: CHAIN_ID,
  ...
  const delegates = useAscensionTokenDelegates({
    args: [address as `0x${string}`],
    enabled: !!address,
    watch: true,
    chainId: CHAIN_ID,
  ...
  const stakedBalance = useAscensionRevenueDistributionTokenBalanceOf({
    args: [address as `0x${string}`],
    enabled: !!address,
    watch: true,
    chainId: CHAIN_ID,
  ...
  const stakedVotes = useAscensionRevenueDistributionTokenGetAssetVotes({
    args: [address as `0x${string}`],
    enabled: !!address,
    watch: true,
    chainId: CHAIN_ID,
  ...
  const stakedDelegates = useAscensionRevenueDistributionTokenDelegates({
    args: [address as `0x${string}`],
    enabled: !!address,
    watch: true,
    chainId: CHAIN_ID,
      ...
      {!isConnected || !chain ? (
        ...
        <>
          <div className="flex rounded bg-black p-2">
            <Button
              variant={token === 'ASCEND' ? 'gray' : 'ghost'}
              className="w-full"
            ...
            <Button
              variant={token === 'xASCEND' ? 'gray' : 'ghost'}
              className="w-full"
          ...
          <div className="space-y-3 rounded border-2 border-purple/50 p-3">
            <div className=" ">
              <h2 className="">{token} balance: </h2>{' '}
              {balance.isSuccess && stakedBalance.isSuccess ? (
                <p>
                  {commify(
                    formatUnits(
                      token === 'ASCEND'
            ...
            <div className=" ">
              <h2 className="">{token} voting power: </h2>{' '}
              {votes.isSuccess && stakedVotes.isSuccess ? (
                <p>
                  {commify(
                    formatUnits(
                      token === 'ASCEND'
                        ? (votes.data as bigint)
                        : (stakedVotes.data as bigint),
                      18
                    ) as string,
                    2
                  )}
            ...
            <div className="">
              <h2 className="">{token} delegate address:</h2>
              {delegates.isSuccess && stakedDelegates.isSuccess ? (
                <p>
                  {shortenString(
                    token === 'ASCEND'
                      ? (delegates.data as string)
                      : (stakedDelegates.data as string),
                    10
                  )}
                </p>
              ) : (
          ...
          {delegating ? (
            <div className="flex items-center gap-1">
              ...
              <Input
                onInput={(e) => setDelegateAddress(e.currentTarget.value)}
